...
#page[
  ...
  #callout(kind: "info")[
    COCONUT WARS（COW）是椰島霸主大地遊戲的最終決戰。你在椰島霸主累積的兵力與領地，將直接轉換為這場海戰的起始資源。
  ]
  ...
  #table(
    ...
    [初始兵力], [椰島霸主期間 4 次世界史更新所累積的兵力總和],
    [初始領地], [椰島霸主結束時，你的小隊所佔領的所有島嶼],
    [無領地者處理], [椰島霸主結束時沒有領地，自動進駐中立小島出發],
    [每回合操作上限], [每回合最多提交 5 個操作（無效操作不計入上限）],
  ...
  #table(
    ...
    [漩渦 R3],
    [1 座],
    [第 3 回合起開放；奇數隊→+2000椰子；偶數隊→+2000兵力；4隊→無效果],
  )

  #v(0.15em)
  #text(size: 8pt, fill: mid-gray)[
    ＊資源點開放：R1/R2 第 2 回合起；R3 第 3 回合起。⭐ 中立小島全程安全。
  ]
]
...
#page[
  ...
  #table(
    ...
    [兵力], [Troops], [戰鬥與移動資源，4 次世界史更新累積而來],
    [椰子], [Victory Points], [最終排名依據，每回合由佔領島嶼產出],
    [領主], [Island Owner], [正式擁有島嶼的隊伍，椰子主要歸領主所有],
    [駐守], [Garrison], [透過 union 進駐盟友島嶼的兵力，非領主身份],
  ...
  #table(
    ...
    [一般大島嶼], [正常戰鬥結算],
    [中立小島 ⭐], [操作*有效*（計入 5 個上限），但 n 兵力*立即損失*作為懲罰，不發動戰鬥],
    [資源點 R1/R2/R3], [操作有效（計入上限），n 兵力立即損失作為懲罰，不發動戰鬥],
  )

  #v(0.3em)
  #callout(kind: "warn")[
    *第 1 回合限制*：攻擊資源點視為無效操作，*不計入 5 個操作上限*，且兵力不損失。
  ]
...
#page[
  ...
  #block(
    ...
    #v(0.15em)
    假設 A 隊想派兵駐守 B 隊的島嶼 D_B，出發地 D_A，派出 n 兵：
    #v(0.15em)
    #grid(
      columns: (1fr, 1fr),
      column-gutter: 10pt,
      block(
        ...
        #text(weight: "bold", fill: accent)[A 隊提交]
        #linebreak()
        #op([union(D_A, B, D_B, n)])
        #linebreak()
        #text(size: 8pt, fill: mid-gray)[P = B（盟友是 B）]
      ],
      block(
        fill: white, stroke: 1pt + accent, radius: 4pt, inset: (x: 6pt, y: 4pt), width: 100%,
      )[
        #text(weight: "bold", fill: accent)[B 隊提交]
        #linebreak()
        #op([union(D_A, A, D_B, n)])
        #linebreak()
        #text(size: 8pt, fill: mid-gray)[P = A；S、E、n 相同]
      ],
    )
    #v(0.15em)
    #text(size: 8.5pt)[
      ▸ 結果：n 兵力從 D_A 移動至 D_B，以「駐守」身份防守 D_B。
    ]
  ]
  ...
  #param-table((
    ...
    [P], [盟友列表], [單一盟友如 [B]，多位如 [B, C]；聯合方皆須列出其他所有參與者],
    [E], [攻擊目標], [敵方島嶼],
    [n], [正整數], [本隊出兵數量],
  ))
...
#page[
  ...
  所有操作在回合開始時「同時」執行，以*回合開始時的兵力*為基準。這意味著：
  ...
  #table(
    ...
    fill: (col, row) => if row == 0 { accent-bg } else if calc.odd(row) { white } else { neutral-bg },
    table.header(
      text(weight: "bold", fill: accent, [情況]),
      text(weight: "bold", fill: accent, [規則]),
    ),
    [有防守方], [兵力較多者獲勝；*平手時防守方勝利*],
    [無防守方], [兵力較少者獲勝（1 隊 > 2 隊 > 3 隊…）；仍平手則互相消滅後重複判定],
    [勝利獎勵], [勝方保留自身兵力＋⌊（敗方兵力總和）×20%⌋；聯合進攻中每位成員*各自獨立*獲得此獎勵],
    [敗方], [出兵兵力全數損失],
  )
  ...
  #table(
    ...
    [僅 A 隊佔領], [A 獲得該島全部 X 椰子],
    [A 隊（領主）＋ B 隊（駐守）],
    [A 獲得 ½X ＋ ½X×（A 兵力比例）；B 獲得 ½X×（B 兵力比例）],
  )
...
#page[
  ...
  #example-box("一", "移動與進攻")[
    *情境*：A 隊有 500 兵在椰島1（己方）；B 隊有 300 兵在椰島2（己方）。

    *本回合操作*：A：#op([moving(椰島1, 椰島3, 200)])；B：#op([attack(椰島2, 椰島1, 250)])

    *戰鬥*：椰島1 — A 防守 500 vs B 攻擊 250 → *A 獲勝*（偽平行：A 的移動不影響防守，防守仍以 500 計）
    ...
    *結算結果*
    #table(
      ...
      stroke: 0.5pt + rgb("#BDBDBD"),
      fill: white,
      [A 隊], [椰島1：500 + ⌊250×20%⌋ = *550 兵*；椰島3：200 兵移入（×1.5 = 300）],
      [B 隊], [進攻失敗，損失 250 兵，剩 50 兵在椰島2],
    )
  ]
  ...
  #example-box("二", "聯盟防守（union）")[
    *情境*：B 有 300 兵在椰島2；A 有 500 兵在椰島1；C（400 兵）計畫攻打椰島2。A+B 協議聯盟。

    *本回合操作*：
    - A：#op([union(椰島1, B, 椰島2, 200)])
    - B：#op([union(椰島1, A, 椰島2, 200)])　（S、E、n 相同，P 互指）
    - C：#op([attack(椰島3, 椰島2, 400)])

    *戰鬥*：椰島2 — C 攻 400 vs（B 防 300 ＋ A 駐守 200 = *500*）→ 防守方勝利
    ...
    *結算結果*
    #table(
      ...
      stroke: 0.5pt + rgb("#BDBDBD"),
      fill: white,
      [B（領主）], [守住椰島2，獲 +80 獎勵兵（⌊400×20%⌋）],
      [A], [200 駐守兵留在椰島2；A 自身也獲 +80 獎勵兵（各自獨立）],
      [C], [損失 400 兵，椰島3 歸零 → 獲 +1000 救濟兵力],
    )
  ]
]
...
#page[
  ...
  #example-box("三", "聯合進攻（union_attack）")[
    ...
    *本回合操作*
    ...
    - A 隊：#op([union_attack(椰島1, [B], 椰島5, 400)])
    - B 隊：#op([union_attack(椰島2, [A], 椰島5, 300)])

    兩方 P 互相列出對方 → 最大互信聯合 A+B 成立

    #v(0.2em)
    *戰鬥結算*：椰島5：A+B 聯合 700 兵 vs C 防守 500 兵 → A+B *獲勝*

    佔領判定：A 出兵 400 > B 出兵 300 → *A 成為椰島5 新領主*，B 以駐守身份留守
    ...
    *結算結果*
    #table(
      ...
      fill: white,
      [隊伍], [身份], [結果],
      [A 隊], [新領主], [400 + ⌊500×20%⌋ = 400 + 100 = *500 兵*，成為椰島5 領主],
      [B 隊], [駐守], [300 + ⌊500×20%⌋ = 300 + 100 = *400 兵*，駐守椰島5],
      [C 隊], [失敗], [損失 500 兵，椰島5 易主],
    )
  ]
  ...
  #example-box("四", "操作衝突")[
    ...
    *本回合操作*
    ...
    - A 隊操作①：#op([moving(椰島1, 椰島3, 150)])　需要 150 兵
    - A 隊操作②：#op([attack(椰島1, 椰島2, 100)])　需要 100 兵

    總需求：150 + 100 = *250 兵 > 200 可用兵力*　→ *操作衝突！*

    #v(0.2em)
    #callout(kind: "warn")[
      ⚠ 操作衝突時，椰島1 的「全部」操作（包含操作①和操作②）均視為無效。不是只有超額的部分，而是所有操作一起作廢。
    ]
  ...
  #align(center)[
    #text(fill: rgb("#9E9E9E"), size: 8.5pt)[
      COCONUT WARS 小隊員說明書　｜　NTU CSIE Camp 大地遊戲　｜　請勿外流
    ]
  ]
]
